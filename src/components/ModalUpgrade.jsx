...
const ModalUpgrade = ({ isOpen, onClose, funcionNombre, onSeleccionarPlan, moneda, onComprarCreditosSoporte }) => {
  ...
  const paquetesSoporte = [
    { id: 'basico', creditos: 5, precioCOP: 9900, precioUSD: 4.99, nombre: t('paqueteBasico') || 'Pack Básico' },
    { id: 'frecuente', creditos: 15, precioCOP: 19900, precioUSD: 9.99, nombre: t('paqueteFrecuente') || 'Pack Frecuente' },
    { id: 'profesional', creditos: 40, precioCOP: 49900, precioUSD: 19.99, nombre: t('paqueteProfesional') || 'Pack Profesional' },
    { id: 'empresarial', creditos: 100, precioCOP: 99900, precioUSD: 39.99, nombre: t('paqueteCorporativo') || 'Pack Empresarial' }
  ];
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-green-500/30 flex flex-col h-full">
            <div className="text-center mb-3">
              <h4 className="text-xl font-bold text-green-400">{t('planStarter') || 'Starter'}</h4>
              <p className="text-2xl font-bold text-white">{moneda?.mostrarCOP ? t('planStarterPrice') || '$29,900' : t('planStarterPriceUSD') || '$9.99'}<span className="text-sm text-gray-400">/mes</span></p>
            ...
            >
              {t('pagar') || 'Pagar'} {moneda?.mostrarCOP ? t('planStarterPrice') || '$29,900' : t('planStarterPriceUSD') || '$9.99'}
            </button>
          </div>
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-cyan-500/30 flex flex-col h-full">
            <div className="text-center mb-3">
              <h4 className="text-xl font-bold text-cyan-400">{t('planPro') || 'Pro'}</h4>
              <p className="text-2xl font-bold text-white">{moneda?.mostrarCOP ? t('planProPrice') || '$79,900' : t('planProPriceUSD') || '$29.99'}<span className="text-sm text-gray-400">/mes</span></p>
            ...
            >
              {t('pagar') || 'Pagar'} {moneda?.mostrarCOP ? t('planProPrice') || '$79,900' : t('planProPriceUSD') || '$29.99'}
            </button>
          </div>
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-purple-500/30 relative flex flex-col h-full">
            ...
            <div className="text-center mb-3 mt-2">
              <h4 className="text-xl font-bold text-purple-400">{t('planBusiness') || 'Business'}</h4>
              <p className="text-2xl font-bold text-white">{moneda?.mostrarCOP ? t('planBusinessPrice') || '$199,900' : t('planBusinessPriceUSD') || '$79.99'}<span className="text-sm text-gray-400">/mes</span></p>
            ...
            >
              {t('pagar') || 'Pagar'} {moneda?.mostrarCOP ? t('planBusinessPrice') || '$199,900' : t('planBusinessPriceUSD') || '$79.99'}
            </button>
          </div>
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-yellow-500/30 flex flex-col h-full">
            <div className="text-center mb-3">
              <h4 className="text-xl font-bold text-yellow-400">{t('planElite') || 'Elite'}</h4>
              <p className="text-2xl font-bold text-white">{moneda?.mostrarCOP ? t('planElitePrice') || '$499,900' : t('planElitePriceUSD') || '$199.99'}<span className="text-sm text-gray-400">/mes</span></p>
            ...
            >
              {t('pagar') || 'Pagar'} {moneda?.mostrarCOP ? t('planElitePrice') || '$499,900' : t('planElitePriceUSD') || '$199.99'}
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-slate-800/30 rounded-lg">
          <p className="text-gray-400 text-xs mb-3 text-center">{t('paquetesEscaneos') || '📦 PAQUETES ADICIONALES DE ESCANEOS'}</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            {paquetesSoporte.map((paquete) => (
              <div key={paquete.id} className="bg-slate-800/50 p-3 rounded text-center">
                <p className="text-cyan-400 font-bold">{paquete.nombre}</p>
                <p className="text-white text-lg font-bold">{moneda?.mostrarCOP ? `$${paquete.precioCOP.toLocaleString()}` : `$${paquete.precioUSD}`}</p>
                <p className="text-gray-500">+{paquete.creditos} {t('escaneos') || 'escaneos'}</p>
                <button
          ...
          <p className="text-gray-500 text-xs mt-3 text-center">{t('paquetesNota') || '💡 Los paquetes se compran dentro de la app y NO están incluidos en el plan mensual'}</p>
        </div>

        {/* 🆕 PAQUETES ADICIONALES DE CRÉDITOS PARA SOPORTE IA CON BOTONES */}
        <div className="mt-6 p-4 bg-slate-800/30 rounded-lg">
          <p className="text-gray-400 text-xs mb-3 text-center">💬 {t('creditPackages') || 'CRÉDITOS ADICIONALES PARA SOPORTE IA'}</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            {paquetesSoporte.map((paquete) => (
              <div key={paquete.id} className="bg-slate-800/50 p-3 rounded text-center">
                <p className="text-purple-400 font-bold">{paquete.nombre}</p>
                <p className="text-white text-lg font-bold">{moneda?.mostrarCOP ? `$${paquete.precioCOP.toLocaleString()}` : `$${paquete.precioUSD}`}</p>
                <p className="text-gray-500">+{paquete.creditos} {t('consultasIA') || 'consultas IA'}</p>
                <button
                  onClick={() => onComprarCreditosSoporte?.(paquete.id, paquete)}
                  className="w-full mt-2 bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white font-bold py-1.5 px-2 rounded-lg transition-all text-xs"
                >
          ...
          <p className="text-gray-500 text-xs mt-3 text-center">{t('creditosNota') || '💡 Los créditos se suman a tu plan actual y NO caducan mensualmente.'}</p>
        </div>
